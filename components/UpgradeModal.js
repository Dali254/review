...
export default function UpgradeModal({ user, onClose, onSuccess }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:600, background:'rgba(20,20,31,0.5)', backdropFilter:'blur(6px)', display:'flex', alignItems:'flex-end', justifyContent:'center' }} onClick={() => step!=='paying' && onClose()}>
      <div style={{ background:'#fff', borderTopLeftRadius:24, borderTopRightRadius:24, borderRadius: window?.innerWidth>520?24:undefined, padding:'12px 28px env(safe-area-inset-bottom,32px)', width:'100%', maxWidth:460, boxShadow:'var(--shadow-lg)', animation:'fadeUp .3s ease' }} onClick={e=>e.stopPropagation()}>
        <div style={{ width:36, height:4, background:'var(--border-strong)', borderRadius:2, margin:'0 auto 22px' }} />

        {step === 'plans' && (
          <>
            <div style={{ textAlign:'center', marginBottom:24 }}>
              <div style={{ width:64, height:64, borderRadius:18, background:'linear-gradient(135deg,#fff7ed,#fed7aa)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 16px' }}>
                <Icon.Zap size={28} style={{ color:'#D97706' }}/>
              </div>
              <h3 style={{ fontSize:21, fontWeight:800, marginBottom:8 }}>Upgrade to Pro</h3>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:24 }}>
              ...
              <div style={{ background:'linear-gradient(135deg,#fff7ed,#fffbeb)', border:'1.5px solid #fed7aa', borderRadius:14, padding:'18px 14px', position:'relative' }}>
                <div style={{ position:'absolute', top:-10, right:10, background:'#D97706', color:'#fff', fontSize:9, fontWeight:800, padding:'3px 9px', borderRadius:20 }}>POPULAR</div>
                <div style={{ fontSize:12, fontWeight:700, color:'#D97706', marginBottom:12, textTransform:'uppercase', letterSpacing:'0.5px' }}>Pro · KES {PRO_PRICE_KES}/mo</div>
                {['Unlimited reviews','KES 15–35 each','Priority payout','Pro badge'].map(f => (
                  <div key={f} style={{ display:'flex', alignItems:'center', gap:7, fontSize:13, color:'var(--text)', marginBottom:8 }}>
                    <Icon.Check size={13} style={{ color:'#D97706', flexShrink:0 }}/>{f}
                  </div>
                ))}
              </div>
            </div>

            <button onClick={() => setStep('phone')} style={{ width:'100%', padding:14, background:'linear-gradient(135deg,#F59E0B,#D97706)', color:'#fff', border:'none', borderRadius:12, fontSize:15, fontWeight:700, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:8, boxShadow:'0 4px 16px rgba(217,119,6,0.3)', marginBottom:10 }}>
              <Icon.Zap size={16}/>Continue — Pay KES {PRO_PRICE_KES}
            </button>
        ...
        {step === 'phone' && (
          <>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:22 }}>
              <h3 style={{ fontSize:18, fontWeight:700 }}>Confirm M-Pesa payment</h3>
            ...
            <div style={{ background:'#fff7ed', border:'1.5px solid #fed7aa', borderRadius:12, padding:'14px 16px', marginBottom:20, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <span style={{ fontSize:14, color:'var(--text-secondary)' }}>Pro subscription (monthly)</span>
              <strong style={{ fontSize:18, color:'#D97706' }}>KES {PRO_PRICE_KES}</strong>
            </div>
            ...
            <div style={{ display:'flex', alignItems:'center', background:'#fff', border:'1.5px solid var(--border-strong)', borderRadius:10, overflow:'hidden', marginBottom:22 }}>
              ...
              <input value={phone} onChange={e=>setPhone(e.target.value.replace(/\D/g,'').slice(0,9))} placeholder="712345678" type="tel" inputMode="numeric" style={{ border:'none', borderRadius:0, flex:1 }}/>
            </div>
            <button onClick={startPayment} disabled={phone.length<9} style={{ width:'100%', padding:14, background: phone.length>=9 ? 'linear-gradient(135deg,#F59E0B,#D97706)' : '#e2e8f0', color: phone.length>=9 ? '#fff' : 'var(--text-muted)', border:'none', borderRadius:12, fontSize:15, fontWeight:700, cursor: phone.length>=9?'pointer':'not-allowed', display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
              <Icon.Smartphone size={16}/>Send STK Push — KES {PRO_PRICE_KES}
            </button>
        ...
        {step === 'success' && (
          <div style={{ textAlign:'center', padding:'16px 0' }}>
            ...
            <p style={{ color:'var(--text-secondary)', fontSize:14, marginBottom:24, lineHeight:1.6 }}>
              KES {PRO_PRICE_KES} received. Unlimited daily reviews unlocked.
            </p>
            <button onClick={() => { onSuccess(); }} style={{ padding:'12px 32px', background:'var(--pink)', color:'#fff', border:'none', borderRadius:12, fontSize:14, fontWeight:700, cursor:'pointer', boxShadow:'0 4px 14px rgba(192,24,95,0.3)' }}>
              Continue reviewing
            </button>
          </div>
        )}
        ...
        {step === 'failed' && (
          <div style={{ textAlign:'center', padding:'16px 0' }}>
            ...
            <button onClick={() => setStep('phone')} style={{ width:'100%', padding:13, background:'var(--pink)', color:'#fff', border:'none', borderRadius:12, fontSize:14, fontWeight:700, cursor:'pointer', marginBottom:10 }}>
              Try again
            </button>
