...
import { BUSINESSES, CATEGORIES, EARN_RATES, logoUrl } from '../data/businesses';
...
function BusinessCard({ biz, onWriteReview }) {
  ...
  return (
    <div style={{ background:'#fff', borderRadius:18, border:'1px solid var(--border)', overflow:'hidden', boxShadow:'var(--shadow)', transition:'box-shadow .2s, transform .2s', display:'flex', flexDirection:'column' }}
    ...
    >
      <Link href={`/business/${biz.id}`} style={{ display:'block', textDecoration:'none' }}>
        <div style={{ height:170, background:`linear-gradient(135deg, ${biz.color}20 0%, ${biz.color}06 100%)`, display:'flex', alignItems:'center', justifyContent:'center', padding:'24px', position:'relative', overflow:'hidden' }}>
          <div style={{ position:'absolute', inset:0, backgroundImage:`radial-gradient(${biz.color}18 1.5px, transparent 1.5px)`, backgroundSize:'22px 22px' }}/>
          {logoOk ? (
          ...
          <div style={{ position:'absolute', top:12, right:12, fontSize:11, color:'var(--text-muted)', fontWeight:500, background:'rgba(255,255,255,0.8)', padding:'3px 9px', borderRadius:20, zIndex:1 }}>{timeAgo(seed)}</div>
          {biz.featured && (
      ...
      <div style={{ padding:'16px 18px 18px', flex:1, display:'flex', flexDirection:'column', gap:10 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:6, marginTop:2 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:7 }}>
            ...
            <span style={{ fontSize:13 }}>Earn: <strong style={{ color:'var(--green)' }}>KES 15 – 35</strong></span>
          </div>
        </div>

        <button onClick={() => onWriteReview(biz)} style={{ marginTop:'auto', width:'100%', padding:'12px', borderRadius:11, border:'none', background:'var(--pink)', color:'#fff', fontSize:14, fontWeight:700, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:8, boxShadow:'0 3px 12px rgba(192,24,95,0.28)', transition:'all .15s' }}>
          <Icon.Send size={14}/>Write Review & Earn KES {EARN_RATES[5]}.00
        </button>
      </div>
    </div>
  );
...
export default function Businesses() {
  const { user, balance, transactions, login, getRemainingTasks, DAILY_FREE_LIMIT, upgradePro } = useUser();
  ...
  function handleWriteReview(biz) {
    ...
    if (remaining <= 0 && user.plan !== 'pro') { setUpgradeOpen(true); return; }
    ...
    <>
      ...
      <div className="page-layout">
        <aside className="sidebar">
          <div style={{ background:'#fff', borderRadius:18, border:'1px solid var(--border)', padding:20, marginBottom:16, boxShadow:'var(--shadow)' }}>
          ...
          <div style={{ background:'#fff', borderRadius:18, border:'1px solid var(--border)', padding:20, marginBottom:16, boxShadow:'var(--shadow)' }}>
            ...
            {[
              ...
              { icon:'Wallet', label:'Balance', value:`KES ${balance.toLocaleString()}`, color:'#7C3AED' },
            ].map(s => {
          ...
          <div style={{ background:'#fff', borderRadius:18, border:'1px solid var(--border)', padding:20, boxShadow:'var(--shadow)' }}>
        ...
        <div>
          <div style={{ marginBottom:22 }}>
            ...
            <p style={{ color:'var(--text-secondary)', fontSize:14, marginBottom:16 }}>Showing <strong>{filtered.length}</strong> businesses — earn KES 15–35 per review</p>
          ...
          {filtered.length === 0 ? (
            ...
            <div className="biz-grid">
              {filtered.map(biz => <BusinessCard key={biz.id} biz={biz} onWriteReview={handleWriteReview}/>)}
            </div>
          )}
        </div>
      </div>
