...
export default function BusinessPage() {
  ...
  const canPublish   = rating > 0 && reviewText.trim().length >= 20;
    ...
    return (
      <div style={{ padding: '24px' }}>
        ...
        {rating > 0 && (
          <div style={{ background: 'rgba(0,200,83,0.08)', border: '1px solid rgba(0,200,83,0.18)', borderRadius: 10, padding: '12px 14px', marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', marginTop: 2 }}>credited instantly, tax on withdrawal</div>
            </div>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <textarea value={reviewText} onChange={e => setReviewText(e.target.value)} placeholder="Share your honest experience..." rows={4} style={{ resize: 'none', fontSize: 14 }} />
          <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', textAlign: 'right', marginTop: 4 }}>
            {reviewText.length} / min 20
          </div>
        </div>
