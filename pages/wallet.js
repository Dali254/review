...
const TX_STYLES = {
  ...
  tax:      { iconBg: 'rgba(255,59,59,0.1)',   iconColor: '#FF3B3B',  amountColor: '#FF3B3B',  IconName: 'ArrowUpRight',sign: '-' },
...
export default function Wallet() {
  ...
  return (
    <>
        ...
        {!user ? (
          ...
          <>
            ...
            <div style={{ position: 'relative', overflow: 'hidden', background: 'rgba(0,200,83,0.07)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: '1px solid rgba(0,200,83,0.16)', borderRadius: 20, padding: '28px 24px', marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 6 }}>Available balance</div>
              <div style={{ fontSize: 'clamp(32px,8vw,48px)', fontWeight: 900, color: '#fff', lineHeight: 1.1, marginBottom: 4 }}>KES {balance.toLocaleString()}</div>
              <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', marginBottom: 6 }}>Total earned: KES {totalEarned.toLocaleString()}</div>
              {/* Tax info banner */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(255,184,0,0.08)', border: '1px solid rgba(255,184,0,0.18)', borderRadius: 8, padding: '8px 12px', marginBottom: 22, fontSize: 12, color: 'rgba(255,184,0,0.9)' }}>
              ...
              <div className="wallet-hero-actions" style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                <button onClick={openWithdraw} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 18px', background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.16)', color: '#fff', borderRadius: 9, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
                  <Icon.CreditCard size={14} /> Withdraw to M-Pesa
                </button>
                <button onClick={() => router.push('/businesses')} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 18px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.09)', color: 'rgba(255,255,255,0.7)', borderRadius: 9, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
            ...
            {/* Daily tasks banner */}
            <div style={{ background: 'rgba(255,255,255,0.04)', backdropFilter: 'blur(14px)', border: `1px solid ${remaining <= 5 ? 'rgba(255,184,0,0.2)' : 'rgba(255,255,255,0.07)'}`, borderRadius: 14, padding: '14px 18px', marginBottom: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
              <div>
            ...
            <div className="wallet-stats" style={{ display: 'grid', gap: 12, marginBottom: 24 }}>
              {[
                ...
                return (
                  <div key={s.lbl} style={{ background: 'rgba(255,255,255,0.04)', backdropFilter: 'blur(14px)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '16px 18px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 8 }}>
                      <IconComp size={12} style={{ color: s.color }} />
                      <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase', letterSpacing: '0.5px', fontWeight: 600 }}>{s.lbl}</span>
                    </div>
            ...
            {/* Transaction list */}
            <div style={{ background: 'rgba(255,255,255,0.03)', backdropFilter: 'blur(14px)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 18, overflow: 'hidden' }}>
              ...
              {transactions.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '48px 16px' }}>
                  <Icon.BarChart size={36} style={{ color: 'rgba(255,255,255,0.08)', marginBottom: 12 }} />
                  ...
                  <button onClick={() => router.push('/businesses')} style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '10px 18px', background: 'rgba(0,200,83,0.1)', border: '1px solid rgba(0,200,83,0.2)', color: '#00C853', borderRadius: 9, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                ...
                transactions.map((tx, i) => {
                  ...
                  return (
                    <div key={tx.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 20px', borderBottom: i < transactions.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.3)', marginTop: 1, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                          <span>{tx.date}</span>
      ...
      {wdOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 400, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}
          ...
          <div style={{ background: 'rgba(13,18,32,0.98)', backdropFilter: 'blur(40px)', border: '1px solid rgba(255,255,255,0.10)', borderRadius: '20px 20px 0 0', padding: '12px 24px env(safe-area-inset-bottom,32px)', width: '100%', maxWidth: 480, animation: 'slideUp 0.3s ease' }}
            ...
            {wdStep === 'form' && (
              <>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 22 }}>
                  ...
                  <button onClick={() => setWdOpen(false)} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.10)', borderRadius: 8, width: 30, height: 30, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'rgba(255,255,255,0.5)' }}><Icon.X size={13} /></button>
                </div>
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 10, overflow: 'hidden' }}>
                    ...
                    <input value={wdPhone} onChange={e => setWdPhone(e.target.value.replace(/\D/g,'').slice(0,9))} placeholder="712345678" type="tel" inputMode="numeric" style={{ border: 'none', background: 'transparent', borderRadius: 0, flex: 1 }} />
                  </div>
                </div>
                {/* Tax preview */}
                {parseInt(wdAmt) >= 100 && (
                  <div style={{ background: 'rgba(255,184,0,0.06)', border: '1px solid rgba(255,184,0,0.18)', borderRadius: 10, padding: '14px 16px', marginBottom: 20 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8, fontSize: 13 }}>
                      <span style={{ color: 'rgba(255,255,255,0.5)' }}>Gross withdrawal</span>
                ...
                <>
                  ...
                  <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: 16, marginBottom: 20 }}>
                    {[
                      ...
                      <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 0', borderBottom: row.label !== 'M-Pesa number' ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
            ...
            {wdStep === 'pending_72' && (
              <div style={{ textAlign: 'center', padding: '16px 0' }}>
                <div style={{ width: 64, height: 64, borderRadius: 18, background: 'rgba(255,184,0,0.1)', border: '1px solid rgba(255,184,0,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
                ...
                {/* 72hr countdown */}
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(255,184,0,0.08)', border: '1px solid rgba(255,184,0,0.2)', borderRadius: 12, padding: '10px 20px', marginBottom: 24 }}>
                ...
                <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, padding: '12px 16px', marginBottom: 20, textAlign: 'left' }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 8 }}>What happens next</div>
                  {[
                    ...
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 0', borderBottom: i < 2 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
                      <div style={{ width: 28, height: 28, borderRadius: 8, background: s.done ? 'rgba(0,200,83,0.12)' : 'rgba(255,255,255,0.05)', border: `1px solid ${s.done ? 'rgba(0,200,83,0.25)' : 'rgba(255,255,255,0.08)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        {<Icon.CheckCircle size={14} style={{ color: s.done ? '#00C853' : 'rgba(255,255,255,0.3)' }} />}
                      </div>
                      <span style={{ fontSize: 13, color: s.done ? 'rgba(255,255,255,0.8)' : 'rgba(255,255,255,0.4)' }}>{s.label}</span>
                    </div>
                  ))}
                </div>
                <button onClick={() => { setWdOpen(false); }} style={{ padding: '11px 32px', background: 'rgba(0,200,83,0.1)', border: '1px solid rgba(0,200,83,0.22)', color: '#00C853', borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
                  Got it, I'll wait
                </button>
              </div>
            )}
