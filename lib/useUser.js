...
export const TAX_RATE_WITHDRAW = 0.16; // 16% tax on withdrawal only
...
export function useUser() {
  ...
  // Call when a review is submitted — returns false if limit hit
  ...
  // Add full earning — NO tax deducted here
  function addEarning(amount, description) {
    ...
    localStorage.setItem(WALLET_KEY, JSON.stringify({
      balance: newBalance, totalEarned: newTotalEarned, totalTax,
    }));
    const newTxs = [
      {
        id: Date.now(),
  ...
  // Withdraw: deduct 16% tax first, then send net to M-Pesa
  // Returns { net, tax } for display
  function withdraw(grossAmount) {
    ...
    localStorage.setItem(WALLET_KEY, JSON.stringify({
      balance: newBalance, totalEarned, totalTax: newTotalTax,
    }));
    const newTxs = [
      {
        id: Date.now() + 1,
        type: 'tax',
        desc: 'Withdrawal tax (16%)',
        amount: -tax,
        date: new Date().toLocaleDateString('en-KE'),
      },
      {
