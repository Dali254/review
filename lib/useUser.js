...
// True if this phone number already has a registered account on this
// device. Used to stop signup from silently creating a duplicate / second
// entry for a number that's already registered — instead the UI should
// redirect that person to "Log in" so they restore their real account.
export function accountExists(phone) {
  return !!loadAccount(phone);
}
...
export function useUser() {
  ...
  // Creates a brand-new account. Returns { success:false, reason:'exists' }
  // WITHOUT writing anything if this phone number is already registered
  // on this device — this is what prevents duplicate entries. The caller
  // (RegistrationGate) should catch that and redirect the person to the
  // Log in tab instead of silently overwriting their real account.
  function signup(userData) {
    const phone = userData?.phone;
    if (!phone) return { success: false, reason: 'invalid' };
    if (accountExists(phone)) {
      return { success: false, reason: 'exists' };
    }
    login(userData); // safe: we just confirmed no account exists yet
    return { success: true };
  }
  ...
  return {
    ...
    tasksToday, loaded,
    login, loginByPhone, signup, accountExists, logout, addEarning, withdraw, upgradePro, setReviewPreference,
